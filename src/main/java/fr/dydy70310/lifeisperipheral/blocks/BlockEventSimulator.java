...
public class BlockEventSimulator extends BlockContainer implements IPeripheralProvider {
	
	
	
	public BlockEventSimulator() {
		super(Material.rock);
	...
	public IPeripheral getPeripheral(World world, BlockPos bpos, EnumFacing side) {
		...
		TileEventSimulator EventSimulator = new TileEventSimulator();
		EventSimulator.pos = bpos;
		EventSimulator.world = world;
		EventSimulator.side = world.getBlockState(bpos).getValue(DIRECTIONBLOCK);
		
		BlockPos pos1;
		if (tile == null)
			return null;
		if (tile instanceof TileEventSimulator) {
