...
public class BlockInventoryInterface extends BlockContainer implements IPeripheralProvider {
	
	
	
	public BlockInventoryInterface() {
		super(Material.rock);
	...
	public IPeripheral getPeripheral(World world, BlockPos bpos, EnumFacing side) {
		...
		TileInventoryInterface chest = new TileInventoryInterface();
		chest.pos = bpos;
		chest.world = world;
		chest.side = world.getBlockState(bpos).getValue(DIRECTIONBLOCK);
		
		BlockPos pos1;
		if (tile == null)
			return null;
		if (tile instanceof TileInventoryInterface) {
	...
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		return new TileInventoryInterface();
	}
	@Override
