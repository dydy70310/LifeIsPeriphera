...
public class BlockAdminChest extends BlockContainer implements IPeripheralProvider {
	...
	public IPeripheral getPeripheral(World world, BlockPos bpos, EnumFacing side) {
		...
		TileAdminChest AdminChest = new TileAdminChest();
		AdminChest.pos = bpos;
		AdminChest.world = world;
		AdminChest.side = world.getBlockState(bpos).getValue(DIRECTIONBLOCK);
		
		BlockPos pos1;
		if (tile == null)
			return null;
		if (tile instanceof TileAdminChest) {
			return AdminChest;
		}
		return null;
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		FMLNetworkHandler.openGui(playerIn, MainLIP.instance, 0, worldIn, pos.getX(), pos.getY(), pos.getZ());
			return true;
	}
